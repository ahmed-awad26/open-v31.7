            ...
            item {
                Card(shape = RoundedCornerShape(24.dp)) {
                    Column(Modifier.fillMaxWidth().padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                        ...
                        SettingsSpacer()
                        OutlinedTextField(value = lockMessage, onValueChange = { lockMessage = it }, modifier = Modifier.fillMaxWidth(), label = { Text("Custom message") })
            ...
            item {
                Card(shape = RoundedCornerShape(24.dp)) {
                    Column(Modifier.fillMaxWidth().padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                        ...
                        SettingsSpacer()
                        OutlinedTextField(value = pin, onValueChange = { pin = it.filter(Char::isDigit); appViewModel.clearError() }, label = { Text(if (settings.hasPin) "Change PIN" else "Set PIN") }, singleLine = true)
